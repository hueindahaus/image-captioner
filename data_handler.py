...
class DataHandler(Dataset):
    def __init__(self, img_folder_path, caption_file_path, img_size=299, gensim_model=None, skip_init=False, val_size=100):
        if not skip_init:
            ...
            # Collect samples
            self.samples, self.stacked_samples = self.collect_samples(val_size)

            # We want to provide data in different ways depending on the mode
            self.train()
    ...
    def __getitem__(self, index):
        ...
        if self.mode == "TRAIN":
            ...
            sample = self.stacked_samples[index]
    ...
    def __len__(self):
        if self.mode == "TRAIN":
            ...
            return len(self.stacked_samples)
        
    def collect_samples(self, val_size):
            ...
            for row in list(csv.reader(csv_file, delimiter='|'))[1:-3]:
                ...
                if img_file_path:
                    ...
                    if caption:
                        ...
                        samples.append((img_file_path, caption))

                        # Populate packed_samples which holds one an image and multiple caption for each entry
                        if len(stacked_samples) < val_size or img_file_path in stacked_samples:
                            if img_file_path not in stacked_samples:
                                stacked_samples[img_file_path] = [caption]
                            else:
                                stacked_samples[img_file_path].append(caption)                    

        return samples, list(stacked_samples.items())
